...
export const GET = withAuth(async (request, { user }) => {
  try {
    ...
    if (error) {
      console.error("Supabase error fetching electric surface pump service reports:", error);
    ...
    const formRecords = data.map((record: any) => {
      ...
      return {
        ...
        companyForm: {
          ...
          name: "Electric Driven Surface Pump Service Report",
    ...
    console.error("API error fetching electric surface pump service reports:", error);
    return NextResponse.json(
...
export const POST = withAuth(async (request, { user }) => {
  try {
    ...
    const motor_connection = getString('motor_connection');
    ...
    if (attachmentFiles.length > 0 && data && data[0]) {
      ...
      for (let i = 0; i < attachmentFiles.length; i++) {
        ...
        if (file && file.size > 0) {
          const filename = `electric-surface/service/${Date.now()}-${sanitizeFilename(file.name)}`;
