...
export const PATCH = withAuth(async (request, { params, user }) => {
  try {
    ...
    const canApprove = await hasPermission(supabase, user.id, "jo_signatory", "approved_by");
    if (!canApprove) {
      return NextResponse.json(
        { error: "You do not have permission to approve as Department Head" },
        { status: 403 }
      );
    }
    ...
    await supabase.from("audit_logs").insert({
      ...
      old_data: {
        approved_by_name: currentRecord.approved_by_name,
        approved_by_signature: currentRecord.approved_by_signature,
      },
      new_data: {
        approved_by_name: fullName,
        approved_by_signature: signatureUrl,
