...
export const POST = withAuth(async (request, { params, user }) => {
  try {
    ...
    // Fetch the JO request
    const { data: joRequest, error: joError } = await supabase
      .from("job_order_request_form")
      .select("*, requester:users!job_order_request_form_created_by_fkey(id, address)")
      .eq("id", id)
      .is("deleted_at", null)
      .single();

    if (joError || !joRequest) {
      return NextResponse.json(
    ...
    const currentStatus = joRequest.approval_status;

    // Determine which level the JO is currently at
    const currentLevel = currentStatus === "pending_level_1"
      ? 1
      : currentStatus === "pending_level_2"
      ? 2
      : currentStatus === "pending_level_3"
      ? 3
      : null;

    if (!currentLevel) {
      return NextResponse.json(
        { success: false, message: "This JO request is not pending approval" },
        { status: 400 }
      );
    }
    ...
    if (filterByAddress) {
      const requesterAddress = joRequest.requester?.address;
      if (!requesterAddress || requesterAddress !== userAddress) {
        return NextResponse.json(
          { success: false, message: "You can only approve JO requests from your branch" },
          { status: 403 }
        );
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Job Order Request ${actionLabel} successfully`,
      data: updatedRecord,
    });
  } catch (error: any) {
