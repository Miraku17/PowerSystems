...
export const GET = withAuth(async (request, { user }) => {
  try {
    ...
    if (error) {
      console.error("Supabase error fetching submersible pump commissioning reports:", error);
    ...
    const approvalMap = await getApprovalsByTable(supabase, "submersible_pump_commissioning_report");
    ...
    const formRecords = data.map((record: any) => {
      ...
      return {
        ...
        companyForm: {
          ...
          name: "Submersible Pump Commissioning Report",
    ...
    console.error("API error fetching submersible pump commissioning reports:", error);
    return NextResponse.json(
...
export const POST = withAuth(async (request, { user }) => {
  try {
    ...
    const submerge_depth = getString('submerge_depth');
    const length_of_wire_size = getString('length_of_wire_size');
    const pipe_size_type = getString('pipe_size_type');
    const pipe_length = getString('pipe_length');
    ...
    const media = getString('media');

    // Other Details
    const commissioning_date = getString('commissioning_date');
    const power_source = getString('power_source');
    const controller_type = getString('controller_type');
    const sump_type = getString('sump_type');
    const controller_brand = getString('controller_brand');
    const pumping_arrangement = getString('pumping_arrangement');
    const controller_rating = getString('controller_rating');
    const others = getString('others');
    ...
    const quality_of_water = getString('quality_of_water');
    const water_temp = getString('water_temp');
    const duration = getString('duration');

    // Comments
    const comments = getString('comments');
    ...
    if (data && data[0]) {
      ...
      await createApprovalRecord(supabase, 'submersible_pump_commissioning_report', data[0].id, user.id);
    }
