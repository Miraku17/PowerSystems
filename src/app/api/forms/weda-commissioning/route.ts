...
export const GET = withAuth(async (request, { user }) => {
  try {
    ...
    if (error) {
      console.error("Supabase error fetching WEDA commissioning reports:", error);
    ...
    console.error("API error fetching WEDA commissioning reports:", error);
    return NextResponse.json(
...
export const POST = withAuth(async (request, { user }) => {
  try {
    ...
    const weda_pump_model = getString('weda_pump_model');
    ...
    const attending_technician_signature = await uploadSignature(
      ...
      `weda/commission/commissioning-attending-technician-${timestamp}.png`
    );
    const approved_by_signature = await uploadSignature(
      serviceSupabase,
      rawApprovedSignature,
      `weda/commission/commissioning-approved-by-${timestamp}.png`
    );
    const acknowledged_by_signature = await uploadSignature(
      serviceSupabase,
      rawAcknowledgedSignature,
      `weda/commission/commissioning-acknowledged-by-${timestamp}.png`
    );
    ...
    // Upload attachments and save to weda_commission_attachments table
    if (attachmentFiles.length > 0 && data && data[0]) {
      ...
      for (let i = 0; i < attachmentFiles.length; i++) {
        ...
        if (file && file.size > 0) {
          const filename = `weda/commission/${Date.now()}-${sanitizeFilename(file.name)}`;
