...
export const GET = withAuth(async (request, { user, params }) => {
  try {
    ...
    addFieldsGrid([
      { label: "Customer's Complaints", value: record.customers_complaints, span: 2 },
      { label: "Possible Cause", value: record.possible_cause, span: 2 },
    ]);
    ...
    addFieldsGrid([
      { label: "Action Taken", value: record.action_taken, span: 2 },
      { label: "Observation", value: record.observation, span: 2 },
      { label: "Findings", value: record.findings, span: 2 },
      { label: "Recommendation", value: record.recommendation, span: 2 },
    ]);
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Electric-Surface-Pump-Service-Report-${
          record.job_order || id
        }.pdf"`,
      },
    });
