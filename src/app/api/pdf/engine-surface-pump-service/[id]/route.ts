...
export const GET = withAuth(async (request, { user, params }) => {
  try {
    ...
    // Helper function for text area fields
    const addTextAreaField = (label: string, value: any) => {
      const text = getValue(value);
      const lines = doc.splitTextToSize(text, contentWidth - 10);
      const boxHeight = Math.max(lines.length * 5 + 12, 20);
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(textGray[0], textGray[1], textGray[2]);
      doc.text(label, leftMargin + 3, yPos + 5);

      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(0, 0, 0);
      doc.text(lines, leftMargin + 3, yPos + 10);
    ...
    addFieldsGrid([
      ...
      { label: "Serial Number", value: record.engine_serial_number },
      { label: "Horse Power", value: record.engine_horse_power },
      { label: "Injection Pump Model", value: record.injection_pump_model },
      { label: "Injection Pump Serial No.", value: record.injection_pump_serial_no },
      { label: "Pump Code", value: record.pump_code },
      { label: "Turbo Charger Brand", value: record.turbo_charger_brand },
      { label: "Turbo Charger Model", value: record.turbo_charger_model },
      { label: "Turbo Charger Serial No.", value: record.turbo_charger_serial_no },
      { label: "Type of Fuel", value: record.type_of_fuel },
      { label: "Engine Oil", value: record.engine_oil },
      { label: "Cooling Additives", value: record.cooling_additives },
    ]);
    ...
    addFieldsGrid([
      { label: "Is the unit within coverage?", value: formatBoolean(record.is_unit_within_coverage) },
      { label: "Is this a warrantable failure?", value: formatBoolean(record.is_warrantable_failure) },
    ]);
    addTextAreaField("Summary Details", record.warranty_summary_details);
      ...
      const signatures = [
        {
          ...
          imageUrl: await resolveSignature(record.performed_by_signature, record.performed_by_name),
        },
        {
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Engine-Surface-Pump-Service-Report-${
          record.job_order || id
        }.pdf"`,
      },
    });
