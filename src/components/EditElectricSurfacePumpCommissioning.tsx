...
export default function EditElectricSurfacePumpCommissioning({
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        const response = await apiClient.get('/forms/electric-surface-pump-commissioning/attachments', { params: { report_id: recordId } });
      ...
      if (response.status === 200) {
        ...
        if (newAttachments.length > 0) {
          ...
          const results = await uploadFiles(
            ...
            { bucket: 'service-reports', pathPrefix: 'electric-surface/commission' }
          );
          results.forEach((r, i) => {
            if (r.success && r.url) {
        ...
        await apiClient.post('/forms/electric-surface-pump-commissioning/attachments', {
          report_id: recordId,
