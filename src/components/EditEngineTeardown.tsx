...
export default function EditEngineTeardown({ data, recordId, onClose, onSaved, onSignatoryChange }: EditEngineTeardownProps) {
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        const response = await apiClient.get('/forms/engine-teardown/attachments', { params: { report_id: recordId } });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.patch(`/forms/engine-teardown/${recordId}`, fd);
      ...
      if (newAttachments.length > 0 || attachmentsToDelete.length > 0 || existingAttachments.length > 0) {
        ...
        try {
          ...
          await apiClient.post('/forms/engine-teardown/attachments', {
            report_id: recordId,
              ...
              <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50 transition-colors cursor-pointer">
                <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600">
                    <label htmlFor="edit-file-upload-engine-teardown" className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500">
                      <span>Upload a file</span>
                      <input id="edit-file-upload-engine-teardown" type="file" accept="image/*,application/pdf" multiple className="sr-only" onChange={async (e) => {
