...
export default function EditSubmersiblePumpService({
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        const response = await apiClient.get('/forms/submersible-pump-service/attachments', { params: { report_id: recordId } });
      ...
      if (response.status === 200) {
        ...
        if (newAttachments.length > 0) {
          ...
          const results = await uploadFiles(
            ...
            { bucket: 'service-reports', pathPrefix: 'submersible/service' }
          );
          results.forEach((r, i) => {
            if (r.success && r.url) {
        ...
        await apiClient.post('/forms/submersible-pump-service/attachments', {
          report_id: recordId,
