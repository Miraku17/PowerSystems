...
export default function ElectricSurfacePumpTeardownForm() {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      motorComponentsAttachments.forEach((attachment) => {
        formDataToSubmit.append('motor_components_files', attachment.file);
        formDataToSubmit.append('motor_components_titles', attachment.title);
      });
      ...
      const response = await apiClient.post('/forms/electric-surface-pump-teardown', formDataToSubmit, {
        headers: {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 bg-gray-50 p-4 md:p-8 rounded-lg border border-gray-100">
            <div className="flex flex-col space-y-4">
              ...
              <SignaturePad label="Draw Signature" value={formData.teardowned_by_signature} onChange={(signature: string) => handleSignatureChange('teardowned_by_signature', signature)} subtitle="Svc Engineer/Technician" />
            </div>
            <div className="flex flex-col space-y-4">
  ...
  return (
    <div className="flex flex-col w-full" ref={dropdownRef}>
      ...
      <div className="relative">
        <input type="text" name={name} value={value} onChange={onChange} onFocus={() => setShowDropdown(true)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900 transition-colors pr-10" placeholder="Select or type a name" />
