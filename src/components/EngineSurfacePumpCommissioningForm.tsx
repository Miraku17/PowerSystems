      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 bg-gray-50 p-6 rounded-lg border border-gray-100">
            <Input label="Name of Reporting Person" name="reporting_person_name" value={formData.reporting_person_name} onChange={handleChange} />
              ...
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-gray-600">
                  <label htmlFor="file-upload-engine-commissioning" className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500">
                    ...
                    <input id="file-upload-engine-commissioning" type="file" accept="image/*,application/pdf" multiple className="sr-only" onChange={async (e) => { if (e.target.files && e.target.files.length > 0) { const files = Array.from(e.target.files); if (attachments.length + files.length > 20) { toast.error('Maximum 20 photos allowed'); e.target.value = ''; return; } const newFiles = []; for (const file of files) { if (!file.type.startsWith('image/') && file.type !== 'application/pdf') { toast.error('Please select image or PDF files'); continue; } const compressed = file.type.startsWith('image/') ? await compressImageIfNeeded(file) : file; newFiles.push({ file: compressed, title: '' }); } if (newFiles.length > 0) setAttachments([...attachments, ...newFiles]); e.target.value = ''; } }} />
                  </label>
