...
import { useGrindexCommissioningFormStore } from "@/stores/grindexCommissioningFormStore";
import { useCreateGrindexCommissioning } from "@/hooks/useGrindexCommissioning";
...
export default function GrindexCommissioningReport() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { formData, setFormData, resetFormData } = useGrindexCommissioningFormStore();
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      toast.success('Grindex Commissioning Report submitted successfully!', { id: loadingToastId });
      setAttachments([]);
      resetFormData();
    } catch (error: any) {
  ...
  return (
    <div className="bg-white shadow-xl rounded-lg p-4 md:p-8 max-w-6xl mx-auto border border-gray-200 print:shadow-none print:border-none">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <div className="flex items-center mb-4">
            <div className="w-1 h-6 bg-green-600 mr-2"></div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 bg-gray-50 p-6 rounded-lg border border-gray-100">
            ...
            <Input label="Pump Weight" name="pump_weight" value={formData.pump_weight} onChange={handleChange} />
          </div>
        </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-3">
                              <input
                                ...
                                className="w-full bg-white border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-green-500 focus:border-green-500 block p-2.5"
                              />
                            </div>
                          </div>
                        </div>
            ...
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50 transition-colors cursor-pointer">
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-gray-600">
                  <label
                    htmlFor="grindex-file-upload"
                    className="relative cursor-pointer bg-white rounded-md font-medium text-green-600 hover:text-green-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-500"
                  >
                    <span>Upload an image</span>
                    <input
                      id="grindex-file-upload"
                      name="grindex-file-upload"
                      type="file"
                      accept="image/*"
                      className="sr-only"
                      onChange={(e) => {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 bg-gray-50 p-4 md:p-8 rounded-lg border border-gray-100">
            ...
            <div className="flex flex-col space-y-4">
              <Select label="Approved By (Supervisors/Manager)" name="approved_by" value={formData.approved_by} onChange={handleChange} options={users.map(user => user.fullName)} />
              <SignaturePad
                label="Draw Signature"
                value={formData.approved_by_signature}
                onChange={(signature: string) => handleSignatureChange('approved_by_signature', signature)}
                subtitle="Supervisors / Manager"
              />
            </div>
            <div className="flex flex-col space-y-4">
        ...
        <div className="flex flex-col-reverse space-y-3 space-y-reverse md:flex-row md:space-y-0 md:justify-end md:space-x-4 pt-6 pb-12">
          ...
          <button type="submit" className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 md:py-3 md:px-10 rounded-lg shadow-md transition duration-150 flex items-center justify-center text-sm md:text-base" disabled={createMutation.isPending}>
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to submit this Grindex Commissioning Report?"
      />
    </div>
  );
}
    ...
    <div className="flex flex-col w-full" ref={dropdownRef}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white text-gray-900 transition-colors pr-10"
