...
export default function GrindexServiceForm() {
  ...
  const handlePumpSelect = (pump: any) => {
    setFormData({
      ...
      engine_serial_no: pump.engineSerialNumber || "",
      kw: pump.kw || "",
      rpm: pump.rpm || "",
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (response.data) {
        toast.success("Grindex Service Report submitted successfully!", {
          id: loadingToastId,
        });
  ...
  return (
    <div className="bg-white shadow-xl rounded-lg p-4 md:p-8 max-w-6xl mx-auto border border-gray-200 print:shadow-none print:border-none">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 bg-gray-50 p-6 rounded-lg border border-gray-100">
            ...
            <Input
              ...
              type="date"
              value={formData.date_commissioned}
              onChange={handleChange}
            />
          </div>
        </div>
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to submit this Grindex Service Report?"
      />
    </div>
  );
}
