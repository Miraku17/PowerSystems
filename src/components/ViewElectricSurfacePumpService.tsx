...
export default function ViewElectricSurfacePumpService({ data, onClose, onExportPDF }: ViewElectricSurfacePumpServiceProps) {
  ...
  const handleApprovalToggle = async (field: 'noted_by' | 'approved_by', checked: boolean) => {
    try {
      await apiClient.patch('/forms/signatory-approval', {
        table: 'electric_surface_pump_service_report',
        recordId: data.id,
        field,
        checked,
      });
