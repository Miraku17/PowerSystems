...
interface ViewEngineSurfacePumpServiceProps {
...
export default function ViewEngineSurfacePumpService({ data, onClose, onExportPDF, onSignatoryChange }: ViewEngineSurfacePumpServiceProps) {
  ...
  const {
    ...
    cancelToggle,
    confirmToggle,
  } = useSignatoryApproval({ table: "engine_surface_pump_service_report", recordId: data.id, onChanged: onSignatoryChange });
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        const attachmentResponse = await apiClient.get('/forms/engine-surface-pump-service/attachments', { params: { report_id: data.id } });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col animate-slideUp overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 bg-white z-10">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h3 className="text-xl font-bold text-gray-900">Engine Driven Surface Pump Service Report</h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-gray-50/50">
          <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-8 max-w-5xl mx-auto">
            ...
            {/* Company Header */}
            <ReportHeader title="Service Report" subtitle="Engine Driven Surface Pump" />
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex items-center mb-4">
                  <div className="w-1 h-6 bg-blue-600 mr-2"></div>
                  <h4 className="text-sm font-bold text-[#2B4C7E] uppercase tracking-wider">Engine Details</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  <Field label="Engine Model" value={data.engine_model} />
                  <Field label="Serial Number" value={data.engine_serial_number} />
                  <Field label="Horse Power" value={data.engine_horse_power} />
                  <Field label="Injection Pump Model" value={data.injection_pump_model} />
                  <Field label="Injection Pump Serial No." value={data.injection_pump_serial_no} />
                  <Field label="Pump Code" value={data.pump_code} />
                  <Field label="Turbo Charger Brand" value={data.turbo_charger_brand} />
                  <Field label="Turbo Charger Model" value={data.turbo_charger_model} />
                  <Field label="Turbo Charger Serial No." value={data.turbo_charger_serial_no} />
                  <Field label="Type of Fuel" value={data.type_of_fuel} />
                  <Field label="Engine Oil" value={data.engine_oil} />
                  <Field label="Cooling Additives" value={data.cooling_additives} />
                </div>
              </div>
                  ...
                  <div className="text-center flex flex-col items-center">
                    <div className="h-24 w-full flex items-end justify-center mb-2 border-b border-gray-300 pb-2">
                      {resolvedPerformedBySig ? (
                        <img src={resolvedPerformedBySig} alt="Service Technician Signature" className="max-h-20 max-w-full object-contain" />
                      ) : (
