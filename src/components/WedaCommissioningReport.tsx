...
import { useWedaCommissioningFormStore } from "@/stores/wedaCommissioningFormStore";
import { useCreateWedaCommissioning } from "@/hooks/useWedaCommissioning";
...
export default function WedaCommissioningReport() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { formData, setFormData, resetFormData } = useWedaCommissioningFormStore();
  const [attachments, setAttachments] = useState<{ file: File; title: string }[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [customers, setCustomers] = useState<any[]>([]);

  const createMutation = useCreateWedaCommissioning();
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      toast.success('WEDA Commissioning Report submitted successfully!', { id: loadingToastId });
      setAttachments([]);
      resetFormData();
    } catch (error: any) {
    ...
    <div className="bg-white shadow-xl rounded-lg p-4 md:p-8 max-w-6xl mx-auto border border-gray-200 print:shadow-none print:border-none">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <div className="flex items-center mb-4">
            <div className="w-1 h-6 bg-blue-600 mr-2"></div>
            <h3 className="text-lg font-bold text-gray-800 uppercase">WEDA Pump Details</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 bg-gray-50 p-6 rounded-lg border border-gray-100">
            <Input label="WEDA Pump Model" name="weda_pump_model" value={formData.weda_pump_model} onChange={handleChange} />
        ...
        <div>
          ...
          <div className="bg-gray-50 p-6 rounded-lg border border-gray-100 space-y-6">
            <TextArea label="Summary (Commissioning of WEDA Pump)" name="summary" value={formData.summary} onChange={handleChange} />
        ...
        <div className="flex flex-col-reverse space-y-3 space-y-reverse md:flex-row md:space-y-0 md:justify-end md:space-x-4 pt-6 pb-12">
          ...
          <button type="submit" className="w-full md:w-auto bg-[#2B4C7E] hover:bg-[#1A2F4F] text-white font-bold py-2 px-4 md:py-3 md:px-10 rounded-lg shadow-md transition duration-150 flex items-center justify-center text-sm md:text-base" disabled={createMutation.isPending}>
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to submit this WEDA Commissioning Report?"
      />
    </div>
  );
}
